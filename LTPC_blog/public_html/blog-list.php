      ...
      <main class="main_blog">
        ...
        <section class="section_blog">  
          ...
          <?php
            ...
            // output variant Y=activ || N=!activ
            function variant($myStr) {    
              $result = "";
                if($myStr == "Y") {
                  $result = "Aktīvs";
                } else {
            ...
            function char_limit($myStr, $limit) {    
              $result = "";
              for ($i=0; $i<$limit; $i++) {
                  $result .= $myStr[$i];
              }
            ...
            $sql = "SELECT id, title, author, DATE_orig, DATE_edit, ENM_status FROM $table";
            ...
            if ($result->num_rows > 0) {
              ...
              while($row = $result->fetch_assoc()) {
                  echo "<tr>";
                  echo "<td>" . char_limit($row["title"], 20) . "</td>"; 
                  echo "<td>" . char_limit($row["author"], 20) . "</td>"; 
                  echo "<td>" . $row["DATE_orig"]=date("Y\. \g\a\d\a j\. F") . "</td>"; 
                  echo "<td>" . $row["DATE_edit"]=date("Y\. \g\a\d\a j\. F") . "</td>"; 
                  echo "<td>" . variant($row["ENM_status"]) . "</td>";
                  echo "<td><a href='blog-form-record-update.php?id=" .$row["id"] . "'><p>Rediģēt</p></a></td>";
                  echo "</tr>";
              } echo "</table>";
