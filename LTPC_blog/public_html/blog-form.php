  ...
  <body>
      ...
      <main class="main_blog">
        <aside>
          ...
          <ul>
            <li>
              <div>
                <a href="blog-form.php"> <img src="img/add.png" alt="img" />
        ...
        <section class="section_blog">  
          ...
          <?php
            ...
            if(empty($id)) {  // new :: record
              ...
              $title  = select_data_mysql(1);
              $subtitle  = select_data_mysql(2);
              $author  = select_data_mysql(3); 
              $content = select_data_mysql(4);
              $id = select_data_mysql(5);
            } echo "ID to uppdate" . $id;
                     
            if ($_SERVER["REQUEST_METHOD"] == "POST") {
              // checking for empty fields 

              if($E == 0) { // if all fields is filled
                ...
                if($_POST['id'] == 0) { echo "id_to_update--" . $id . "--";  data_update_mysql(); } 
                if($_POST['id'] != 0) { echo "id_to_insert--" . $id . "--";  insert_data_mysql(); }
              }
            }
          ?>
    ...
    <?php 
      ...
      function data_update_mysql(){
        ...
        $sql = "UPDATE $table SET title='$title', subtitle='$subtitle', author='$author', DATE_edit=NOW(), content='$content' WHERE id='$id'"; // ID ?????
        
        if ($conn->query($sql) === TRUE) {
          echo '<script>message()</script>'; // [MESSAGE]
          echo "--date update--" . $title;
          echo "--ID--" . $id;
